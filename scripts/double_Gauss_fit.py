...
if __name__ == "__main__":
    ...
    x_draws = smhm_func.random_draw_inverse_CDF(
        theta_true, smhm_func.func_double_Gaussian, -5.0, 8.0, N_draws, key
    )
    ...
    bestfit_theta, loss_hist = smhm_func.gradient_descent(
        ...
        x=bins,
        y_target=data,
        model=smhm_func.model_double_Gaussian_ndhist_10K,
        N_steps=10_000,
        stepsize=1e-5,
    )
    ...
    # Second panel: histogram and fits
    ax[1].hist(x_draws, bins=bins, alpha=0.5, label="raw data")
    ax[1].plot(bin_centers, data, label="smooth data")

    ax[1].plot(
        bin_centers,
        smhm_func.model_double_Gaussian_ndhist_10K(theta_init, bins),
        ":k",
        label="initial guess",
    )
    ax[1].plot(
        bin_centers,
        smhm_func.model_double_Gaussian_ndhist_10K(bestfit_theta, bins),
        "--g",
        label="final model",
    )
    ax[1].legend()
